...
export const SCENARIOS = {
  normal: {
    ...
    def: () => base(),
    get explication() { return t(`Conduction AV normale : AH ≈ 80 ms, HV ≈ 40 ms, allongement progressif de l'AH avec la précocité de l'extrastimulus, sans saut. En stimulation ventriculaire, la conduction rétrograde passe par le nœud AV : activation atriale concentrique, la plus précoce sur le His, et décrémentielle. Aucune tachycardie n'est inductible.`, `Normal AV conduction: AH ≈ 80 ms, HV ≈ 40 ms, with gradual AH prolongation as the extrastimulus becomes more premature, and no jump. During ventricular pacing, retrograde conduction is over the AV node: concentric atrial activation, earliest at the His, and decremental. No tachycardia is inducible.`); },
  },
  double: {
  ...
  trin: {
    ...
    def: () => avec(sans(base(), 'nav'),
      ...
      { id: 'lente', a: 'cs9', b: 'his', nodale: true, ab: nod(190, 120, 90, 240), ba: null }),
    cible: 'lente',
    get explication() { return t(`Réentrée intranodale typique : descente par la voie lente (saut d'AH à l'induction), remontée par la voie rapide. VA très court (A et V quasi simultanés, VA < 70 ms (du début du QRS à l'A le plus précoce, ici au His)), activation atriale rétrograde concentrique, la plus précoce sur le His. Une ESV délivrée quand le His est réfractaire ne modifie pas l'atrium. Après entraînement ventriculaire : réponse V-A-V, PPI − TCL > 115 ms et SA − VA > 85 ms. L'adénosine l'arrête. Traitement : ablation de la voie lente.`, `Typical AV nodal re-entry: anterograde conduction over the slow pathway (AH jump at induction), retrograde over the fast pathway. Very short VA (A and V almost simultaneous, VA < 70 ms (from QRS onset to the earliest A, here at the His)), concentric retrograde atrial activation, earliest at the His. A PVC delivered when the His is refractory does not affect the atrium. After ventricular entrainment: V-A-V response, PPI − TCL > 115 ms and SA − VA > 85 ms. Adenosine terminates it. Treatment: slow pathway ablation.`); },
  },
  'trin-atyp': {
  ...
  jonctionnelle: {
    get contexte() { return t(`Homme de 21 ans, tachycardie à QRS fins favorisée par l'effort, parfois dissociée des P.`, `21-year-old man with a narrow-QRS tachycardia favoured by exertion, sometimes dissociated from the P waves.`); },
    position: 'cryo-his',
...
export const POSITIONS = [
  ...
  { id: 'og-lat', get nom() { return t('Oreillette gauche inféro-latérale (en regard du SC 3-4)', `Inferolateral left atrium (facing CS 3-4)`); }, a: 'cs3', v: 'lvl', stim: 'cs3', cibles: ['foyer'] },
