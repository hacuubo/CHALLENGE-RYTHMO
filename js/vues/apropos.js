...
export function vueAPropos(app) {
  ...
  app.innerHTML = `
    ...
    <section class="carte">
      ...
      <p>${t('Base de questions :', 'Question bank:')} <b>${base.questions.length}</b> questions — version ${esc(base.version)}${base.date ? ` ${t('du', 'of')} ${new Date(base.date).toLocaleDateString(locale())}` : ''}.<br><span class="note">${parType}</span></p>
      <p class="note">${t('Les questions s\'appuient uniquement sur des sources scientifiquement validées (recommandations ESC/EHRA/HRS/ACC/AHA, HAS, articles indexés, manuels techniques officiels) et ont été relues par un rythmologue pour un français clair et naturel. Chaque question indique sa date de relecture.',
        'The questions rely solely on scientifically validated sources (ESC/EHRA/HRS/ACC/AHA guidelines, HAS, indexed articles, official technical manuals) and were reviewed by an electrophysiologist. Each question shows its review date. The English version is a translation of the original French questions.')}</p>
      ...
      <p class="note">${t('<a href="presentation.html">Présentation de l\'application</a> : thèmes couverts, méthode, recommandations de référence et questions fréquentes.',
        '<a href="presentation.html">About the app</a>: topics covered, method, reference guidelines and frequently asked questions.')}</p>
    </section>
    <section class="carte"><h2>${t('Tracés ECG', 'ECG tracings')}</h2>
      <p class="note">${t('Les bandes de rythme (dérivation DII) sont synthétiques, générées par l\'application pour illustrer rythmes et dysfonctions de stimulation.', 'The rhythm strips (lead II) are synthetic, generated by the app to illustrate rhythms and pacing malfunctions.')} ${nbReels ? `${t(`Les ${nbReels} ECG 12 dérivations sont de vrais enregistrements issus de`, `The ${nbReels} 12-lead ECGs are real recordings from`)} <a href="https://physionet.org/content/ptb-xl/" target="_blank" rel="noopener noreferrer">PTB-XL</a> (PhysioNet, ${t('licence', 'licence')} CC BY 4.0)${t(' :', ':')} Wagner P, Strodthoff N, Bousseljot RD et al. PTB-XL, a large publicly available electrocardiography dataset. Sci Data 2020;7:154 (<a href="https://doi.org/10.1038/s41597-020-0495-6" target="_blank" rel="noopener noreferrer">doi</a>)${t(' ;', ';')} Goldberger AL et al. PhysioBank, PhysioToolkit, and PhysioNet. Circulation 2000;101:e215-e220. ${t('Tracés rééchantillonnés à 250 Hz, ligne de base corrigée.', 'Resampled at 250 Hz, baseline corrected.')}` : ''}</p>
    </section>
