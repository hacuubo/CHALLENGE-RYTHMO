...
export function vueSimulateur(app) {
  ...
  app.innerHTML = `
    ...
    <details class="carte simu-guide">
      ...
      <ul>
        <li><b>Voies</b> : DII et V1 en surface ; OD haute ; His (A, H, V) ; sinus coronaire du proximal (9-10, ostium) au distal (1-2, paroi latérale de l'oreillette gauche) ; VD apex. « S » en haut = stimulus capturant, « s » = non capturant.</li>
        <li><b>Extrastimulus</b> : train de S1 (ex. 8 × 600 ms) puis S2, S3, S4 (0 = désactivé). Diminuez S2 par pas de 10 ms (bouton « S2 − 10 », qui stimule aussitôt) : saut de l'AH ≥ 50 ms = double voie nodale ; période réfractaire quand l'extrastimulus ne capture plus ou ne conduit plus.</li>
        ...
        <li><b>ESV His-réfractaire</b> : site VD, détection sur le His, Nb S1 = 0, S2 ≈ 10-20 ms : le stimulus tombe quand le His vient d'être activé. Si l'atrium suivant est avancé, une voie accessoire participe (le nœud AV, réfractaire, ne peut pas conduire).</li>
        <li><b>Entraînement ventriculaire</b> : salve VD à un cycle 10 à 40 ms plus court que la tachycardie, vérifiez que l'atrium suit, arrêtez. Réponse <b>V-A-V</b> (réentrée utilisant le nœud AV ou une voie accessoire) ou <b>V-A-A-V</b> (tachycardie atriale). Mesurez PPI − TCL sur la voie VD : &gt; 115 ms en faveur d'une réentrée intranodale, &lt; 115 ms d'une réentrée utilisant une voie accessoire (critère validé surtout pour distinguer réentrée intranodale et voie accessoire septale).</li>
        <li><b>Adénosine</b> : bloc AV transitoire environ 1,5 s après l'injection ; une tachycardie qui persiste malgré le bloc n'utilise pas le nœud AV.</li>
      ...
      <p class="note">Modèle pédagogique simplifié : les intervalles sont réalistes mais le cœur est réduit à quelques sites. Concept inspiré du simulateur svtsim (S. Iravanian) ; code et scénarios originaux. Critères : Michaud GF et al., JACC 2001;38:1163-7 (PPI − TCL) ; Knight BP et al., JACC 1999;33:775-81 (V-A-A-V) ; Josephson ME, Clinical Cardiac Electrophysiology.</p>
    </details>`;
  ...
  function image(maintenant) {
    ...
    if (maintenant - st.dernierMaj > 250) {
      ...
      const msg = [st.fige ? '⏸ Figé' : '', st.salve ? `Salve ${r.site.toUpperCase()} ${st.salve.cl} ms` : '', ad ? 'Adénosine' : '', tach.active ? `Tachycardie (cycle ${Math.round(tach.cycleV ?? tach.cycleA)} ms)` : ''].filter(Boolean).join(' · ');
