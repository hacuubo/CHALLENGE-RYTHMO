...
export function vueSimulateur(app) {
  ...
  $('#valider').onclick = () => {
    ...
    const traitement = st.scenario === 'fa'
      ...
      : cible.length
        ? (sc.ablationOptionnelle && !tire ? t(`✓ Abstention ou traitement médical acceptables ; si ablation : ${esc(posNom)}.`, `✓ Watchful waiting or medical therapy acceptable; if ablation: ${esc(posNom)}.`)
    ...
    $('#verdict').innerHTML = `<div class="retour ${juste ? 'ok' : 'ko'}"><h3>${juste ? t('Bon diagnostic !', 'Correct diagnosis!') : t('Ce n\'est pas ça.', 'Not quite.')} ${t('Note :', 'Score:')} ${note} / 10</h3>
